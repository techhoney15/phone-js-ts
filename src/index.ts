...
export function validatePhoneNumber({
  ...
  const COUNTRY_DATA: any = countryPhoneData.find(
    (country) =>
  ...
  if (COUNTRY_CODE && CALLING_CODE) {
    if (
      ...
      COUNTRY_DATA?.countryCallingCode !== CALLING_CODE
    ) {
      return {
        status: false,
        error:
  ...
  if (!COUNTRY_DATA?.phone_number_lengths.includes(PHONE.length)) {
    return {
      status: false,
      error:
  ...
  if (
    COUNTRY_DATA?.mobile_begin_with &&
    Array.isArray(COUNTRY_DATA.mobile_begin_with)
  ) {
    const validPrefixes = COUNTRY_DATA.mobile_begin_with;
    const phoneStartsWithValidPrefix = validPrefixes.some((prefix: string) =>
      PHONE.startsWith(prefix)
    );
    if (!phoneStartsWithValidPrefix) {
      return {
        status: false,
        error: `Phone number must start with one of: ${validPrefixes.join(
          ", "
        )} of country ${COUNTRY_DATA.countryName}`,
        data: null,
      };
    }
  }
  return {
    status: true,
    error: null,
    message: `Phone number is valid for ${COUNTRY_DATA?.countryName} (${COUNTRY_DATA?.countryCode})`,
    data: {
